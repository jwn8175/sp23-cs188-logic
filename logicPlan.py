...
def sentence3() -> Expr:
    ...
    return conjoin([expr1, expr2, pacmanBorn_0]) 
    "*** END YOUR CODE HERE ***"
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    return assignments == model # TODO also unsure if this is the right way  
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    # if there a wall at (x, y) then pacman is not at (x, y) at time t  
    for coord in all_coords: 
        x, y = coord 
        pacphysics_sentences.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t)) 
    
    # pacman is at exactly one non-wall (x, y) at time t 
    ...
    # append results of successorAxioms 
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    # add pacman's intial location to KB 
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0)) 

    for t in range(50): 
        print("Current timestep: {}".format(t)) 

        # pacman can only be at one location at timestep t 
        ...
        if (t > 0): 
            transitions = [pacmanSuccessorAxiomSingle(coord[0], coord[1], t, walls_grid) for coord in non_wall_coords] 
            KB += transitions 
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    # add pacman's initial location and indicate there is no wall there 
